...
<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center gap-1 pt-1">
      ...
      <x-arrow />
      <x-nav-link href="{{ route('personal-docente-edad.index') }}" :active="true">
  ...
  <section class="flex flex-col w-full px-20 pt-10 pb-32">
            ...
            <div class="flex flex-col gap-4 px-4 py-8">
              <div class="flex flex-col gap-2 mb-3">
                ...
                <p class="text-secondary">
                  ...
                  este <a href="{{ route('formato.importacion', ['name' => 'formato-personal-docente_edad']) }}"
                    class="underline">archivo.</a>.
                </p>
              </div>
              <form action="{{ route('personal-docente-edad.import') }}" method="post" enctype="multipart/form-data">
                @csrf
        ...
        <article>
          ...
          <x-modal name="create">
            <div class="px-4 py-8">
              ...
              <form method="post" action="{{ route('personal-docente-edad.store') }}">
                @csrf
                @method('post')
                ...
                <div class="flex flex-col gap-1 mt-5">
                  <x-input-label for="grupo_edad" :value="__('Grupo de Edad')" />
    ...
    <section class="w-full mt-1 text-green-900 min-h-[40px]">
            ...
            <tr class="{{ $item->status ? '' : 'opacity-40' }}">
              ...
              <td>
                <div class="flex gap-2">
                  <a href="{{ route('personal-docente-edad.show', ['personal_docente_edad' => $item->id]) }}"
                    class="btn-primary">Ver</a>
                  @if (Auth::check() && Auth::user()->role->role == 'Administrador')
                    <a href="{{ route('personal-docente-edad.edit', ['personal_docente_edad' => $item->id]) }}"
...
<script>
  $(document).ready(datatable({
    ...
    props: {
      orderBy: [4, 'desc'],
      scroll: 'true',
      fileName: 'Personal docente por grupo de edad',
