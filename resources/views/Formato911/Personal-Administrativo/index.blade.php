...
<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center gap-1 pt-1">
      ...
      <x-arrow />
      <x-nav-link href="{{ route('personal-administrativo.index') }}" :active="true">
      ...
      <section class="flex items-center justify-end w-full gap-1">
        ...
        <article>
          <x-primary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'create')">
            {{ __('Nuevo Personal Administrativo') }}
          </x-primary-button>
          <x-modal name="create">
            <div class="px-4 py-8">
              <h2 class="title">Crear nuevo Personal administrativo</h2>
              <form method="post" action="{{ route('personal-administrativo.store') }}">
                @csrf
                @method('post')
                ...
                <x-forms.input-data nameHombres="directivo_h" nameMujeres="directivo_m" name="Directivos" />
                <x-forms.input-data nameHombres="docente_h" nameMujeres="docente_m" name="Docentes" />
                <x-forms.input-data nameHombres="docente_investigador_h" nameMujeres="docente_investigador_m"
                  name="Docentes investigadores" />
                <x-forms.input-data nameHombres="investigador_h" nameMujeres="investigador_m" name="Investigadores" />
                <x-forms.input-data nameHombres="auxiliar_investigador_h" nameMujeres="auxiliar_investigador_m"
                  name="Auxiliares investigadores" />
                <x-forms.input-data nameHombres="administrativos_m" nameMujeres="administrativos_h"
                  name="Administrativos" />
                <x-forms.input-data nameHombres="otros_h" nameMujeres="otros_m" name="Otros" />
      ...
      <table class="table stripe" id="personalAdministrativo" style="width: 100%">
        <thead class="text-sm bg-gray-900 text-gray-50">
          <tr>
            ...
            <th>Campus</th>
            <th>Año</th>
            <th>Total directivos</th>
            <th>Total docentes</th>
            <th>Total docente investigador</th>
            <th>Total investigador</th>
            <th>Total auxiliar investigador</th>
            <th>Total administrativos</th>
            <th>Total otros</th>
            <th>Opciones</th>
          </tr>
        </thead>
        <tbody class="text">
          @foreach ($personalAdministrativo as $item)
            <tr class="{{ $item->status ? '' : 'opacity-40' }}">
              ...
              <td>{{ $item->anio }}</td>
              <td>{{ $item->directivo_t }}</td>
              <td>{{ $item->docente_t }}</td>
              <td>{{ $item->docente_investigador_t }}</td>
              <td>{{ $item->investigador_t }}</td>
              <td>{{ $item->auxiliar_investigador_t }}</td>
              <td>{{ $item->administrativo_t }}</td>
              <td>{{ $item->otros_t }}</td>
              <td>
                <div class="flex gap-2">
                  <a href="{{ route('personal-administrativo.show', ['personal_administrativo' => $item->id]) }}"
                    class="btn-primary">Ver</a>
                  @if (Auth::check() && Auth::user()->role->role == 'Administrador')
                    <a href="{{ route('personal-administrativo.edit', ['personal_administrativo' => $item->id]) }}"
